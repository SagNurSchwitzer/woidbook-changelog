...
export default function Changelog(props) {
    ...
    const iosItems = props.changelog.ios.map((text) =>
        <li key={Math.random()}>{text.type == "ADDED" ? <Badge variant="success">ADDED</Badge> : text.type == "REMOVED" ? <Badge variant="danger">REMOVED</Badge> : <Badge variant="warning">FIXED</Badge>} {text.content}</li>
    );
